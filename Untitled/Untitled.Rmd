...
title: An introductory tutorial of labeled property graphs for psychological data
titlerunning: Graph Databases
authors: 
  - firstname: Juan
    particle: C.
    lastname: Correa
...
# abstract: |
abstract: |
  Labeled property graphs offer a flexible framework for representing complex psychological data structures, such as multilevel designs, longitudinal assessments, and dynamic networks. This tutorial introduces graph databases and their labeled property graphs as an alternative to relational databases, illustrating how they enable efficient modeling of interconnected psychological phenomena. We provide a step-by-step guide for implementing property graphs using Neo4j, including schema design, query patterns, and integration with R/Python for analysis. Practical examples drawn from open psychological datasets demonstrate performance and flexibility gains over traditional relational approaches. Reproducible materials, including code and data, are available via OSF to facilitate adoption and replication.
...
As nodes and edges can represent distinct behaviors, contingencies, and interactions, graph database management systems represent a relevant methodological tool for behavioral sciences.
I will elaborate upon these implications immediately.

# Methodological implications of graph databases
...
An important methodolgical implication embedded in graph database models is their performance when handling connected data compared to relational databases that rely on tabular structures [@Robinson2015].
...
Nonetheless, big data research is no longer outside professional considerations [@vezzoli2023].
Such considerations, however, have been tackled by network scientists in charge of analyzing urban traffic at an individual level by tracking the movements of hundreds of thousands of drivers every two hours [@Gonzalez2008].
...
By default any instance created in neo4j is "stopped" mode and the user should start it by clicking one button.
The data import is straightforward using the import option depicted with a red circle in the left panel of neo4j (see Figure [4](#fig:F4)).
...
Sketching a graph database model is the next step after a successful data import in Neo4j. Given the benefits of the labeled property graph, an initial model can be modified multiple times. These modifications are important because they reflect the analyst’s expertise and highlight critical nodes and relationships to be mapped.
An initial graph database model can consider the most elementary schema focused on the relationship between restaurants and customers (Figure [4](#fig:F5)).
