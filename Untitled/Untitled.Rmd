...
In psychological research, many phenomena are inherently relational and multi‑layered: people interact with others, form impressions, and generate outcomes that unfold over time.
These relational processes are difficult to represent using rectangular tables, which require data duplication, artificial identifiers, and complex join operations across multiple files.
LPG models overcome these limitations by allowing each node and relationship to carry arbitrary key–value properties, multiple labels, and flexible schemas that naturally adapt to the structure of psychological processes.

To illustrate, consider the schema in Figure 2 .
This dataset contains information at three distinct levels: (1) stable participant characteristics (e.g., age, ethnicity, sex), (2) interaction‑level impressions recorded during each brief date (e.g., attraction, perceived similarity, desire to say “yes”), and (3) longitudinal follow‑up responses across subsequent weeks for pairs who continued interacting.
These layers of data are typically stored in separate tables with many NAs and require extensive preprocessing before analysis.
But in an LPG, they are represented as interconnected components of a single, coherent graph structure.
3.1 Entities as Nodes Three principal node types are used in the LPG representation:
...
3.2 Relationships as Edges Edges define how these nodes are linked and encode psychological structure directly:
...
3.3 Why This Matters for Psychological Research This structure offers several methodological advantages:
...
Follow‑up nodes can be traversed sequentially, enabling queries such as “How early do committed feelings emerge among mutually matched pairs?”
