...
An initial graph database model can consider the following schema depicted in Figure [5](#fig:F5).

```{r F5, echo=FALSE, fig.cap="A visual representation of the initial schema about online food delivery", fig.align="center", out.height="180px", out.height="200px", out.width="380px", fig.pos="H"}
knitr::include_graphics("/home/jcc/Documents/GitHub/NOSQLINBS/F5.png")
```

This graph model is the result of a series of cypher syntaxes like the following one

``` cypher
CYPHER 5 CREATE CONSTRAINT `Typical_Traffic Afternoon_TrafficPM_uniq` IF NOT EXISTS
FOR (n: `TrafficPM`)
REQUIRE (n.`Typical Traffic Afternoon`) IS UNIQUE;
```
