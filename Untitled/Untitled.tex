...
In psychological research, many phenomena are inherently relational and
multi‑layered: people interact with others, form impressions, and
generate outcomes that unfold over time. These relational processes are
difficult to represent using rectangular tables, which require data
duplication, artificial identifiers, and complex join operations across
multiple files. LPG models overcome these limitations by allowing each
node and relationship to carry arbitrary key--value properties, multiple
labels, and flexible schemas that naturally adapt to the structure of
psychological processes.

To illustrate, consider the schema in Figure 2 . This dataset contains
information at three distinct levels: (1) stable participant
...
These layers of data are typically stored in separate tables with many
NAs and require extensive preprocessing before analysis. But in an LPG,
they are represented as interconnected components of a single, coherent
graph structure.

\subsection{Entities as Nodes Three principal node types are used in the
LPG
representation:}\label{entities-as-nodes-three-principal-node-types-are-used-in-the-lpg-representation}

Participant nodes represent individuals in the study and store stable
demographic properties such as age, ethnicity, and sex (from the
codebook variables DemAge, DemEthn, and
sex\_M1\_F2)..docx.pdf).docx.pdf) {[}OSF \_ Vari\ldots2015).docx
\textbar{} PDF{]}

DateEvent nodes represent each dyadic interaction and store the full set
of impression‑formation ratings provided by participants during
...
variables in the dataset..docx.pdf).docx.pdf) {[}OSF \_
Vari\ldots2015).docx \textbar{} PDF{]}

FollowUp nodes represent week‑level longitudinal responses, including
relationship status, romantic interest, sexual desire, and commitment
indicators (e.g., FUStatus, FURomInterest, FUSexDesire, FUCommitted),
captured only for participants who matched and continued
interacting..docx.pdf) {[}OSF \_ Vari\ldots2015).docx \textbar{} PDF{]}

\subsection{Relationships as Edges Edges define how these nodes are
linked and encode psychological structure
directly:}\label{relationships-as-edges-edges-define-how-these-nodes-are-linked-and-encode-psychological-structure-directly}

HAD\_DATE edges connect Participants to the DateEvents in which they
took part, indicating the rater's perspective on that interaction.

WITH edges connect the DateEvent back to the other participant, allowing
the system to explicitly represent dyadic structure.

FOLLOWED\_UP edges link Participants to their FollowUp nodes,
representing relational trajectories across weeks.

Optionally, a MUTUAL\_MATCH\_WITH edge can be created between two
Participants when the mutmatch property equals 1, providing a direct
representation of reciprocal liking---a relationship that is invisible
in rectangular data but naturally expressed in graph form.

\subsection{Why This Matters for Psychological Research This structure
offers several methodological
advantages:}\label{why-this-matters-for-psychological-research-this-structure-offers-several-methodological-advantages}

Psychological data retain their natural relational structure. Dyadic
interactions are modeled as shared events rather than duplicated rows,
preserving the topology of social and romantic processes as they occur
in the real world.

Temporal processes become navigable. Follow‑up nodes can be traversed
...
Missingness is handled structurally, not numerically. Participants who
did not match simply lack follow‑up nodes; no NA encoding is required.

The model is inherently extensible. Additional constructs, such as
personality traits or behavioral logs, can be attached as new nodes or
properties without reconfiguring the existing database schema.
...
\section{\#\# Instance creation and data
