...
Neo4j offers two free options: ``Neo4j Community Edition'' and ``Neo4j
AuraDB Free,'' as well as two licensed options called ``Neo4j
enterprise'' and ``Neo4j AuraDB Professional/Enterprise.'' Behavioral
scientists can download any of its free options from its official
website and install it in their laptops, desktop computers, or
laboratory servers. As the official documentation of neo4j provides
...
\subsection{Instance creation}\label{instance-creation}

After installation, the instance creation is the second step behavioral
scientists can follow. This is a top-level operation for setting up a
new database environment, and involves allocating resources (memory,
CPU, storage), defining a database version, and setting up initial user
credentials by clicking one button. When creating a new instance, neo4j
asks the user to provide a name as part of the instance details. The
user is asked to provide a password of eight characters as a security
check for further interactions. The instance created is called ``OFD,''
an acronym from ``Online Food Delivery.'' Figure \hyperref[fig:F3]{3}
depicts this instance which by default is ``stopped.'' The user should
start the instance by clicking the button Start instance that is at the
top right of the window. This action will create two default databases
